...
namespace details {
    ...
    std::enable_if_t<std::is_integral_v<std::decay_t<T>>, typename types::integral_type>
    ...
    template<typename T, typename types, typename array_type, typename object_type>
    std::enable_if_t<std::is_convertible_v<std::decay_t<T>, typename types::string_type>,
    ...
    template<typename T, typename types, typename array_type, typename object_type>
    std::enable_if_t<std::is_convertible_v<std::decay_t<T>, array_type>, array_type>
    to_compatible_value(T&& t) {
        return std::forward<T>(t);
    }
    ...
    template<typename T, typename types, typename array_type, typename object_type>
    std::enable_if_t<std::is_convertible_v<std::decay_t<T>, object_type>, object_type>
    to_compatible_value(T&& t) {
        return std::forward<T>(t);
    }
    ...
    template<typename T>
    struct entity_size<T, std::enable_if_t<std::is_same_v<typename std::decay_t<T>::size_type,
                                                          decltype(std::decay_t<T>().size())>>> {
...
public:
    ...
    using value_t =
        std::variant<std::monostate, bool_t, integral_t, floating_t, string_t, array_t, object_t>;
    template<typename T>
...
public:
    ...
    basic_property(T&& t) :
        value(details::to_compatible_value<T, types, array_t, object_t>(std::forward<T>(t))) {}
    ...
    template<typename T,
             typename _ = std::enable_if_t<is_convertible_v<T> ||
                                           std::is_same_v<std::decay_t<T>, std::decay_t<this_t>>>>
    ...
    std::size_t size() const {
        ...
        return std::visit(
            [](auto&& e) {
                ...
                return details::entity_size<decltype(e)>::get(e);
            },
            value);
    }
    ...
    template<typename type, std::enable_if_t<std::is_floating_point_v<type> &&
                                                 std::is_convertible_v<type, floating_t>,
                                             int> = 0>
    ...
    template<
        typename T,
        std::enable_if_t<std::is_integral_v<T> && std::is_convertible_v<T, integral_t>, int> = 0>
    ...
    template<typename T,
             std::enable_if_t<std::is_floating_point_v<T> && std::is_convertible_v<T, floating_t>,
                              int> = 0>
    ...
    template<typename T,
             std::enable_if_t<std::is_same_v<T, array_t> || std::is_same_v<T, object_t> ||
...
std::basic_ostream<c, traits>& operator<<(std::basic_ostream<c, traits>& os,
    ...
    std::visit(
        details::overloaded{[](const std::monostate&) {},
...
using property = basic_property<details::types<char>>;
